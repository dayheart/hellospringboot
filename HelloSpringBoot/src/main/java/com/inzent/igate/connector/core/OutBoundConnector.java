			...
			*/
			...
			if(mciProtocol.equalsIgnoreCase("HTTP")) {
				...
				if(reqObj!=null) {
					...
					Field enumConst = SysHeader.class.getField("OGTRAN_GUID");
					...
					XLog.stdout(String.format("##### REQ_DATA_TYPE [%s]", requestData.getClass().getName()));
					
					Object o_tmsg = method.invoke(enumObj, requestData);
					XLog.stdout(String.format("##### ENUM_VAL: [%s]", o_tmsg));
					
					String OGTRAN_GUID = String.format("%s", o_tmsg);
					
					enumConst = SysHeader.class.getField("INFC_ID");
					enumObj = enumConst.get(null);
					XLog.stdout(String.format("##### ENUM_FIELD [%s]", enumObj));
					method = SysHeader.class.getMethod("getField", new Class[] { byte[].class, String.class });
					XLog.stdout(String.format("##### ENUM_METHOD [%s]", method));
					o_tmsg = method.invoke(enumObj, requestData, "UTF-8");
					XLog.stdout(String.format("##### ENUM_VAL: [%s]", o_tmsg));
					String INFC_ID = String.format("%s", o_tmsg);
					
					int random_sleep_milisec = (int) (Math.random() * 13) + 12;        
			        String txcode="txcode" + random_sleep_milisec;
					
					String TMSG_SVC_ID = txcode;
					
					url = url + "OGTRAN_GUID=" + OGTRAN_GUID.trim() + "&INFC_ID=" + INFC_ID.trim() + "&TMSG_SVC_ID=" + TMSG_SVC_ID.trim();
					...
					if(corProtocol.equalsIgnoreCase("HTTP")) {
						...
						if(mciOut.equalsIgnoreCase("octet-stream")) {
							...
							method = null;
							// check SysHeader.class.isEnum()
							enumConst = SysHeader.class.getField("TMSG_SYNCZ_SECD");
							...
							o_tmsg = method.invoke(enumObj, b_response);
